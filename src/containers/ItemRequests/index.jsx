      ...
      <div style={{
        ...
        <div style={{
          ...
          <Container style={{
            ...
            {/* Removido: exibição do nome do usuário */}
            {/*
            {user && (
              <div style={{ color: '#eaf6fb', marginBottom: 12 }}>
            ...
            */}
            ...
            {(user?.papel !== 'admin' || user?.papel === 'admin') && (
              <div style={{
                ...
                <h2 style={{ color: '#00eaff', marginBottom: 18 }}>Nova Requisição</h2>
                <RequestForm onSubmit={handleSubmit}>
                  {/* Se admin, permite selecionar o usuário */}
                  {user?.papel === 'admin' && (
                    <Label>
                      Usuário
                      <select
                      ...
                      {/* Removido: Mostra o ID do usuário selecionado */}
                    </Label>
                  )}
                  {/* Removido: Resumo dos dados a serem enviados */}
                  <Label>
                    Item
                    <select
            ...
            {/* Lista de requisições em tabela */}
            <h3 style={{
              color: '#00eaff',
            ...
            <TableContainer component={Paper} sx={{ backgroundColor: '#181c24', boxShadow: '0 0 15px #00eaff22' }}>
              <Table>
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ color: '#00eaff' }}>Nº</TableCell>
                    <TableCell sx={{ color: '#00eaff' }}>Item</TableCell>
                    <TableCell sx={{ color: '#00eaff' }}>Quantidade</TableCell>
                    <TableCell sx={{ color: '#00eaff' }}>Status</TableCell>
                    <TableCell sx={{ color: '#00eaff' }}>Solicitante</TableCell>
                    {user?.papel === 'admin' && (
                      <TableCell sx={{ color: '#00eaff' }}>Ações</TableCell>
                    )}
                  </TableRow>
                </TableHead>
                <TableBody>
                  {loading ? (
                    <TableRow>
                      <TableCell colSpan={user?.papel === 'admin' ? 6 : 5}>Carregando...</TableCell>
                    </TableRow>
                  ) : requests.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={user?.papel === 'admin' ? 6 : 5}>Nenhuma requisição encontrada.</TableCell>
                    </TableRow>
                  ) : (
                    requests.map((req, idx) => {
                      const itemNome = itens.find(i => i.id === req.item_id)?.nome || req.item_id;
                      // Busca o nome do usuário pelo usuario_id ou requisitante_id
                      let solicitante = '';
                      const usuarioId = req.usuario_id || req.requisitante_id;
                      if (usuarios.length > 0 && usuarioId) {
                        const usuarioObj = usuarios.find(u => String(u.id) === String(usuarioId));
                      ...
                      // Verifica se o usuário pode excluir (é o dono e não está aprovada)
                      const podeExcluir = (
                        (user?.id === req.requisitante_id || !req.requisitante_id) &&
                        req.status !== 'aprovado'
                      );
                      // Novo: admins podem excluir qualquer requisição não aprovada
                      const adminPodeExcluir = user?.papel === 'admin' && req.status !== 'aprovado';
                      return (
                        <TableRow key={req.id}>
                          <TableCell sx={{ color: '#eaf6fb' }}>{(req.id || '').slice(-6).toUpperCase()}</TableCell>
                          <TableCell sx={{ color: '#eaf6fb' }}>{itemNome}</TableCell>
                          <TableCell sx={{ color: '#eaf6fb' }}>{req.quantidade}</TableCell>
                          <TableCell>
                            <span style={{
                              ...
                              borderRadius: 12,
                              fontWeight: 'bold',
                              background: req.status === 'aprovado'
                                ? '#388e3c'
                          ...
                          <TableCell sx={{ color: '#eaf6fb' }}>{solicitante}</TableCell>
                          {user?.papel === 'admin' && (
                            <TableCell>
                              {/* Botões de ação */}
                              {req.status === 'pendente' && (
                              ...
                              {/* Botão de excluir para admin em requisições não aprovadas */}
                              {adminPodeExcluir && (
                                <button
                                  style={{
                                    background: '#c91407',
                          ...
                          {/* Botão de excluir para o usuário dono da requisição e não aprovada */}
                          {user?.papel !== 'admin' && podeExcluir && (
                            <TableCell>
                              <button
                                style={{
                                  background: '#c91407',
